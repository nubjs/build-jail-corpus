...
⛔ **And read the `REPLAY CONFIRMED` check's own green just as narrowly: it means "this particular guard did not regress", not "no replay happened".** The store-eviction and persistent-leaf paths remain covered only by their own evictions. Neither check subsumes the other and neither covers all four replay paths, so a run with both green has ruled out two of them, not replay in general.

⭑ **`playwright-chromium@0.17.0` is the one package that exercises the persistent-leaf purge**, because an `ms-playwright` artifact surviving between arms is what produced its historical false `OVER-PREDICTED: dropping 'network'`. Re-measured 2026-08-06 with the leaf purge in place: the synth grant `{"write":{"userHome":true},"network":true}` verified on the first arm and BOTH descent arms failed — `drop-no-network` and `drop-no-write-userHome` each `rc=1` — giving `MINIMAL`. The capability that once falsely dropped is now proven necessary. That is the strongest evidence available that the leaf purge holds, and it is a stronger case than either `falsify.mjs` case runs, so wiring it in as a third case is the obvious next step. (Its arms carry `gate-vacuous`, so the artifact gate proves nothing for it — the arms failed on **rc**, which `arm-falsifiability.mjs` did NOT flag as vacuous for this package, so the failure is real evidence.)
