...
- **A memory-mapped write — UNKNOWN, not absent, and not yet measured even once.** The known-answer fixture has an `mmap` row and it has always reported ABSENT. That was read as an ETW blind spot through two investigations, and it is not one: the fixture's grandchild never performed a memory-mapped write. `MemoryMappedFile.CreateFromFile` was called with `$null` as the map name, PowerShell marshals `$null` for a .NET `String` as the empty string, and the API rejects that — so every run threw `Map name cannot be an empty string.` into a `catch` whose output nobody read, while the child gated the expectation on the grandchild's exit code, which is 0 either way. An expectation that can never be satisfied is a standing false finding rather than an assertion. Both are fixed (the two-argument path overload, plus a read-back of the byte, plus gating on that confirmation) and `probes/win-viability/mmap-variants.ps1` exists to separate "not observable" from "the lazy flush landed after we stopped listening" — but the measurement has not run. **Treat memory-mapped writes as an unquantified risk until it does.**
