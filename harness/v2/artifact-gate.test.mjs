...
//
// ⛔ AND IT IS THE WHOLE BUILD TREE, NOT ONE FILE. The first cut excused `build-tmp-napi-v<N>/config.gypi`
// alone. Re-measuring `@discordjs/opus@0.10.0` on linux-x64 at the pinned Nub with that rule in place
// returned `rc=0 artifacts=750/750 missing=161`, every entry a merely-shorter
// `build-tmp-napi-v3/Release/.deps/**/*.o.d` (2919B < 3052B, 3037B < 3170B, …) — the same `.d` class
// `build/` has excused since `lmdb-store@2.0.0-alpha2`, at the same unanchored directory. So the
// parametrisation below covers the family, and the excusal keys on the build DIRECTORY.
for (const f of ['build-tmp-napi-v8/config.gypi', 'build/CMakeFiles/CMakeConfigureLog.yaml',
  'build/build.ninja', 'build-tmp-napi-v3/Release/.deps/Release/obj.target/libopus/celt/bands.o.d',
  'build-tmp-napi-v3/Makefile', 'build-tmp-napi-v8/Release/obj.target/binding/src/binding.o']) {
...
test('the re-measured @discordjs/opus shape passes: 750/750 present, only shorter `.o.d` files', () => {
  // The second half of the same defect, at its measured sizes. Under the `config.gypi`-only rule this
  // arm read `missing=161` with the identical shortfall digest at every rung and the package landed
  // NO-STATE-PASSED for an install that exited rc=0 with every artifact present.
  const dep = 'build-tmp-napi-v3/Release/.deps/Release/obj.target/libopus/deps/opus/celt/bands.o.d';
  const obs = tree('obs-opus', { 'index.js': 'x', [dep]: 'x'.repeat(3052),
    'prebuilds/opus.node': 'x'.repeat(4096) });
  const r = gate(obs, tree('arm-opus', { 'index.js': 'x', [dep]: 'x'.repeat(2919),
    'prebuilds/opus.node': 'x'.repeat(4096) }));
  assert.equal(r.code, 0, `the re-measured NO-STATE-PASSED case must now pass:\n${r.out}`);
  assert.match(r.out, /missing=0 /, `and report a clean manifest:\n${r.out}`);
});
