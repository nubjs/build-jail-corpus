...
verify () {
  ...
  # ⛔ ONE FILE, ONE WRITE. Epoch 29 added a SECOND `printf ... > "$v/.npmrc"` at the top of this
  ...
  # below the ladder. It adds a catalog `baseline`, which is a document-level key, so an arm that
  # carried it by accident would be running a different experiment from every other arm while
  ...
  ( cd "$v"
    ...
    # ⛔ THE PROMOTION PROBE'S PER-ARM REAL HOME, AND THE ORDER OF THESE TWO LINES IS LOAD-BEARING.
    # `XDG_CACHE_HOME` is pinned to the value the ORIGINAL `$HOME` implies BEFORE `HOME` moves, because
    # `sandbox_homes` falls back to `$HOME/.cache` only when the variable is unset — so repointing HOME
    # first would silently relocate the store, the persistent private jail home and the tool caches,
...
descend () {
  ...
  #
  # ⛔⛔ THE ARM LIST COMES FROM `descent-terms.mjs` AND NO LONGER FROM THREE INLINE COPIES. This was a
  # `node -e` snippet here, a second one in `measure-macos.sh`, and a third shape in
  # `measure-windows.mjs` — which is exactly how Linux came to emit the bare `network` / `write.deps`
  # names for the whole life of its descent while the other two spelled them `no-*`. One module, three
  # callers, one vocabulary; `descent-vocabulary.test.mjs` pins the delegation on all three.
  #
  ...
  # recorder cannot parse. The module answers that shape with NO write term plus a recorded skip
  # reason, rather than with a refusal of the whole descent, so the rung's OTHER term (`no-network`)
  # is measured instead of being discarded along with it. `read` is a SCOPE rather than a map of
  # scopes, so the whole key still drops under ONE name — there is deliberately no `no-read-<scope>`
  # spelling mirroring `no-write-<scope>`, because a second vocabulary is what the recorder's
  # unparsed-name guard exists to prevent.
  #
  # ⛔ A REFUSAL FROM THE MODULE MEANS NO DESCENT, NEVER A GUESSED ONE. It throws only on a grant shape
  ...
  if [ -z "$CAPS" ]; then
    ...
    for cap in $CAPS; do
      # ⛔ THE APPLIER IS SHARED WITH THE NAME GENERATOR, AND IT THROWS RATHER THAN NO-OPPING. The
      # inline version here did `delete g.write[k]`, which on a STRING `write` silently changes
      # nothing — so an arm built from a name the generator should never have produced would have run
      # the UNNARROWED grant, passed trivially, and been recorded as proof the capability was
      # droppable. `descent-terms.mjs`'s `narrow()` refuses a drop it cannot genuinely apply.
      if ! SUB=$(node "$HERE/descent-terms.mjs" --narrow "$g0" --drop "$cap"); then
        echo "     !! DESCENT-TERMS could not narrow $g0 by '$cap' — arm SKIPPED, not scored"
    ...
    if [ "${DROPPED_N:-0}" -ge 2 ]; then
      # Same shared applier as the per-arm drops above — a joint grant computed by a second copy of
      # the delete logic is a second chance for the two to disagree about what a name means.
      if ! JOINT=$(node "$HERE/descent-terms.mjs" --narrow "$g0" --drop "$NARROWER"); then
        echo "  => JOINT-NARROW INCONCLUSIVE — the joint grant could not be computed from$NARROWER;"
...
# KEPT". So `rc` cannot fail for a dropped entry, `artifact-gate.mjs` is scoped to the package's own
# directory and never looks in the home, and `denial-witness.mjs` scores refusals a dropped promotion
# does not produce. A `{"writePaths":[…]}` grant flattens to ZERO capability tokens in
# `publish-guard.mjs`, so `hasRedArm` is false by construction and EVERY narrowing to such a grant is
# withheld — measured, `@clerk/shared@2.9.2` is exactly that record on darwin today.
#
# The pair below is the missing detector. Both arms run the ordinary `verify`, so nothing about the
# jail, the eviction or the replay guards changes; the ONE variable is the declaration, and the ONE
# thing read afterwards is whether the declared directory reached that arm's own real home.
#
# ⛔ EACH ARM GETS ITS OWN REAL HOME, AND WITHOUT THAT THE DROP ARM COULD NOT GO RED. Promotion only
# ever ADDS (`promote_declared_path` renames where the destination is absent), so a control arm that
# promoted into the shared `$HOME` would leave the directory sitting there for the drop arm to find —
# a green that could not have been red, arrived at through the very instrument meant to prevent one.
# `$ARM_PROMOTION_HOME` repoints HOME for the arm's nub invocation and for nothing else.
#
# ⛔ AND IT PINS `XDG_CACHE_HOME` WHILE DOING IT. `build_jail.rs::sandbox_homes` resolves the cache as
# `XDG_CACHE_HOME` first and `$HOME/.cache` only as a fallback, so moving HOME with that variable
# unset would move the STORE, the private jail home and the tool caches with it — a cold arm whose
# failure would read as a denial. The two must move independently, and this is what makes them.
ARM_PROMOTION_HOME=
promotion_probe () {
  local g="$1" ch="$ROOT/promo-control-home" dh="$ROOT/promo-drop-home" dropg
  # No declaration, or a grant that already writes the real home: the module says so and stops. The
  # marker still prints — a silent skip is indistinguishable from a driver that has no probe at all,
  # which is the state every `writePaths` record was already in.
  if [ -z "$(node "$HERE/promotion-probe.mjs" --grant "$g" --entries)" ]; then
    node "$HERE/promotion-probe.mjs" --grant "$g" --score --platform linux
    return 0
  fi
  # The applier is `descent-terms.mjs`'s, shared with every other drop in this driver, and it THROWS
  # rather than producing a no-op narrowing. A refusal means no probe, never a guessed one.
  ...
  # ⛔ THE ARMS' rc IS DELIBERATELY NOT READ. A failed control arm produces no directory and the gate
  # reports UNPROVEN-CONTROL on its own; reading rc here would add a second, differently-scoped
  # verdict for the same fact and the two would eventually disagree.
  node "$HERE/promotion-probe.mjs" --grant "$g" --score \
    --control-home "$ch" --drop-home "$dh" --platform linux
}
...
#
# ⛔⛔ THE TERMINAL RUNG CHANGES TWO VARIABLES AT ONCE, AND EVERY `write:"disk"` RECORD IN THIS CORPUS
# WAS PRODUCED BY IT. `write:"disk"` is not a wider sandbox — it is NO sandbox. ONE function makes it
# (`nub-sandbox/src/compiler/preset.rs`, `relax_fs_to_full_disk`) and it flips three things together:
# `entries.clear()` (which discards the secret/`.env` floor), `default_effect = Allow`, and
# `tmp = Shared`. `linux_landlock.rs` then emits a single `/` `FullDisk` rule and stands down. So a
# package that passes ONLY there may need a wide write scope, or may be failing for a
# sandbox-compatibility reason no path grant can fix — and a pass/fail ladder cannot tell those apart.
#
# This arm is the missing state BETWEEN them: the last confined rung's grant, widened by a catalog
# `baseline` of concrete read-write directories. A baseline entry compiles to an ordinary ALLOW under
# the build jail's `default_effect = Deny` (`preset.rs`, `build_jail_surface`), so
# `fs_confines(fs) = default_effect != Allow || !entries.is_empty()` — the SAME predicate in all three
# backends — stays TRUE and Landlock stays engaged. The path set and the never-a-root-glob guard live
# in `confined-wide.mjs`.
#
# ⛔ IT IS A PROBE AND NOT A RUNG, WHICH IS WHAT MAKES IT FAIL-CLOSED. Its widening comes from a GLOBAL
# baseline, and the shipped per-package grant vocabulary has no spelling for one — `catalog_v2::Reach`
# is `None | Scopes | Disk` and nothing else. Publishing a grant this arm passed at would hand the
# catalog an entry NARROWER than the package was measured to need, which is the under-granting
# direction. So it records and returns; the ladder then walks the terminal rung exactly as before, and
# the terminal rung still decides the published grant. Nothing here can narrow a record.
ARM_CONFINED_WIDE=
confined_wide_probe () {
...
do
  ...
  if [ "$frc" -eq 0 ]; then
    ...
    # the rung is a BUNDLE like every other, and skipping the descent threw away its SECOND term along
    # with the first. MEASURED on the committed corpus 2026-09-01: all 75 `write:"disk"` records carry
    # `overPredictedBy: []` and `minimality: null`, and all 75 also carry `network: true` — a
    # capability no arm has ever tried to drop, on the records where the filesystem axis is already
    # gone. `descent-terms.mjs` emits no write term for the string reach and `no-network` for the rest,
    # so the term that CAN be measured is, and the one that cannot says why.
    #
    # ⛔ THE WRITE AXIS IS NOT SILENTLY DROPPED, IT IS DELEGATED. Every write reach narrower than
    # `Reach::Disk` that the catalog can spell is a `Scopes` value — i.e. rungs 0 and 1, which this
    # very loop already ran and which already FAILED on this package. So a `no-write-disk` arm would
    # re-run a refuted hypothesis under a new name. What is still open — needs-the-whole-filesystem
    # versus fails-under-confinement-for-another-reason — is `confined_wide_probe`'s question, and it
    # ran a few lines above on this same rung.
    descend "$g" ladder
    # A LADDER rung never carries a `writePaths` declaration — the rungs are literal grant strings a
    # few lines above and none of them has the key — so the probe declines on `no-declaration` here
    # and costs one `node` invocation. It is called anyway rather than being made conditional on the
    # grant's shape: a driver where the probe runs on one path and not another is how the marker comes
    # to be present for synthesized records and silently absent for ladder ones.
    promotion_probe "$g"
    exit 0
  fi
done
